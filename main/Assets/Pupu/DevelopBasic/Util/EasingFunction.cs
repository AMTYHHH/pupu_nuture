...
namespace EasingFunc
{
    public abstract class Easing
    {
        ...
        public static float CosPulse(float t){
            float normalizedT = Mathf.PI * t * 2;
            return (-Mathf.Cos(normalizedT)+1) * .5f;
        }

        public static float Impulse(float t, float k = 12){
            float h = k*t;
            return h*Mathf.Exp(1.0f-h);
        }
        public static float pcurve( float x, float a=5.0f, float b=5.0f){
